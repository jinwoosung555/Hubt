...
GriefWindow = Library:CreateWindow({ Title = "Ken Hub | Rivals", Footer = "Ken Hub • Mobile", Center = true, AutoShow = true, NotifySide = "Right", ShowCustomCursor = false })
Window = GriefWindow
...
--// Ken Hub Mobile UI
--// UI-only layer: compact scaling, floating K toggle, and touch-friendly bottom navigation.
pcall(function()
    local Players = game:GetService("Players")
    ...
    local old = guiParent:FindFirstChild("KenHubMobileUI")
    if old then old:Destroy() end

    local ui = Instance.new("ScreenGui")
    ui.Name = "KenHubMobileUI"
    ...
    -- Compact the main Obsidian window on small/mobile screens.
    task.defer(function()
        pcall(function()
            local holder = Window.MainFrame or Window.Holder
            if holder and holder:IsA("GuiObject") then
                local scale = Instance.new("UIScale")
                scale.Scale = UIS.TouchEnabled and 0.72 or 0.88
                scale.Name = "KenHubMobileScale"
                scale.Parent = holder
            end
        end)
    end)
    ...
    k.Position = UDim2.new(1, -72, 0.62, 0)
    k.BackgroundColor3 = Color3.fromRGB(35, 35, 48)
    k.Text = "K"
    ...
    k.Activated:Connect(function()
        ...
        pcall(function()
            local holder = Window.MainFrame or Window.Holder
            if holder then holder.Visible = visible end
        end)
    end)
    ...
    local function activateTab(label)
        -- Tries to activate an existing Obsidian tab/button by visible text.
        local root = (Window.MainFrame or Window.Holder or ui)
        for _, obj in ipairs(root:GetDescendants()) do
            if obj:IsA("TextButton") and string.lower(obj.Text or "") == string.lower(label) then
